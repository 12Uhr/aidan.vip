...
namespace Engine
{
	...
	void ClanTag()
	{
		...
		case 2:
		{
			SetMyClanTag("aidan.vip", "aidan.vip");
			break;
		}
		case 3:
		{
			SetMyClanTag("aidan.vip \r", "aidan.vip only");
			break;
		}
		case 4:
		{
		...
		case 6:
		{
			...
			int ServerTime = (float)pPlayer->GetTickBase() * 3 * Interfaces::GlobalVars()->interval_per_tick;
			...
			case 8:
			{
				SetMyClanTag("DAN.VIP  ", "");
				break;
			}
			case 9:
			{
				SetMyClanTag("IDAN.VIP ", "");
				break;
			}
			case 10:
			{
				SetMyClanTag("AIDAN.VIP", "");
				break;
			}
			case 11:
			{
				SetMyClanTag(" AIDAN.VI", "");
				break;
			}
			case 12:
			{
				SetMyClanTag("  AIDAN.V", "");
				break;
			}
			case 13:
			{
	...
	void TimeClanTag()
	{
		...
		SetMyClanTag(time.c_str(), "Zeus");
	}
	void ClanTagApply(const char* TagName)
	{
		SetMyClanTag(TagName, "Zeus");
	}
	void ChatSpamRegular()
	{
		std::vector<std::string> chatspam = { "aidan.vip" };
	...
	void ChatSpamRandom()
	{
		std::vector<std::string> chatspamsss = {
			"can this get 50 likes?",
			"ok, now that was epic",
			"i gave away wife for internet conect, pls giv me knif!",
			"that wasn't very epic of you" ,
			"epic swag",
			"please follow fortnite lads on ig",
			"soulja boy the video game coming soon to the xbox 720 and ps5!",
			"mad cuz bad btw",
			"sit dog :DDDD",
			"dame tu cosita ah ah",
			"please",
			"skbsajbkdaskjbdksjabkj"
		};
