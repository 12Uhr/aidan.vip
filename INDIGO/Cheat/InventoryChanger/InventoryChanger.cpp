...
void CInventoryChanger::ApplyMusic(CMsgSOCacheSubscribed::SubscribedType* pInventoryCacheObject, uint32_t MusicIndex)

{
	CSOEconItem Music;
	Music.set_account_id(Interfaces::SteamUser()->GetSteamID().GetAccountID());
	...
	Music.set_quality(4);
	Music.set_level(1);
	Music.set_def_index(58);
	Music.set_inventory(20000 + (int)MusicIndex);
	Music.set_id(20000 + (int)MusicIndex);
	CSOEconItemEquipped* EquippedState = Music.add_equipped_state();
	EquippedState->set_new_class(0);
	EquippedState->set_new_slot(54);


	auto KitID = Music.add_attribute();
	int KitIDAttribute = (int)MusicIndex;

	KitID->set_def_index(166);
	KitID->set_value_bytes(&KitIDAttribute, 4);

	pInventoryCacheObject->add_object_data(Music.SerializeAsString());
}
