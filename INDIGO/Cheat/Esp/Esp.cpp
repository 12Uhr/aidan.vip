			...
			if (pModel)
			{
				...
				if (pModelName)
				{
					...
					if (WorldToScreen(pEntity->GetRenderOrigin(), vEntScreen))
					{
						...
						{
							g_pRender->Text((int)vEntScreen.x, (int)vEntScreen.y, true, true, Color::Green(), "[ BOMB ]");
						}
...
void CEsp::DrawPlayerEsp(CPlayer* pPlayer)
{
	...
	if (Settings::Esp::isdefusing && pPlayer->m_pEntity->m_bIsDefusing() && pPlayer->Team == TEAM_CT)
	{
	...
	if (Settings::Esp::esp_Weapon && !pPlayer->WeaponName.empty())
	{
		...
		string replace1 = "M4A1-S";
		string search2 = "DesertEagle";
		string replace2 = "Deagle";
